...
This is a headless rework of the [Streamlit gmail tutorial](../README.md): same
RAG engine (`embedchain`), but split into scripts that OpenClaw can call, with a
**persisted** vector DB so questions are fast and cheap.

```
You ──(Telegram)──▶ OpenClaw ──matches the "gmail-chat" skill──▶ ask_gmail.py
                       ▲                                              │
...
| `skills/gmail-chat/SKILL.md` | The OpenClaw skill that wires it to Telegram |
...
# copy this openclaw_integration/ folder's contents into /opt/gmail-rag, e.g.:
cp -r advanced_llm_apps/chat_with_X_tutorials/chat_with_gmail/openclaw_integration/* /opt/gmail-rag/
cd /opt/gmail-rag
```
...
Follow steps 3 of the [original README](../README.md) to create an OAuth client
for the Gmail API, then download the client secret as **`credentials.json`** into
...
> **Note:** the OpenClaw skill files aren't committed to this repo yet — the
> steps below are the plan for once they land. Until then, the scripts work
> standalone (run `ask_gmail.py` directly).
...
# Common locations:
ln -s /opt/gmail-rag/skills/gmail-chat ~/.claude/skills/gmail-chat
#   or
ln -s /opt/gmail-rag/skills/gmail-chat ~/.codex/skills/gmail-chat
```

If `/opt/gmail-rag` isn't your install path, edit the paths inside
`skills/gmail-chat/SKILL.md` to match.
...
OpenClaw recognizes these as `gmail-chat` requests, runs `ask_gmail.py`, and
